 ...
 * ./exportAllRuleAssets.js -i 'DQ Experiments' -d 'DQ' -f /tmp/extract.tgz
 */
...
const cmdExportIA = envCtx.istool + " export" +
    " -af " + envCtx.authFile +
    " -ar '" + path.dirname(argv.filepath) + path.sep + path.basename(argv.filepath, '.tgz') + ".isx'" +
...
const cmdExportBG = envCtx.istool + " glossary export" +
    " -af " + envCtx.authFile +
    " -f '" + path.dirname(argv.filepath) + path.sep + path.basename(argv.filepath, '.tgz') + ".xml'" +
    " -format XML -allpoliciesrules -includeassignedassets -includestewardship -includelabeledassets -includeTermHistory";
...
if (typeof argv.dsname !== 'undefined' && argv.dsname !== null && argv.dsname !== "") {
  ...
  const cmdExportDS = envCtx.istool + " export" +
      " -af " + envCtx.authFile +
      " -ar '" + path.dirname(argv.filepath) + path.sep + path.basename(argv.filepath, '.tgz') + ".isx'" +
...
const cmdTGZ = "tar zcv -C " + path.dirname(argv.filepath) + " -f '" + argv.filepath + "' '" + path.posix.basename(argv.filepath, '.tgz') + ".isx' '" + path.posix.basename(argv.filepath, '.tgz') + ".xml'";
...
shell.rm(path.dirname(argv.filepath) + path.sep + path.basename(argv.filepath, '.tgz') + ".isx");
shell.rm(path.dirname(argv.filepath) + path.sep + path.basename(argv.filepath, '.tgz') + ".xml");
