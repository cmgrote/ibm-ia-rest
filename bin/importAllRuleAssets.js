...
let cmdImportIA = envCtx.istool + " import" +
    " -af " + envCtx.authFile +
...
if (typeof argv.dsname !== 'undefined' && argv.dsname !== null && argv.dsname !== "") {
  ...
  const cmdImportDS = envCtx.istool + " import" +
      " -af " + envCtx.authFile +
      " -ar '" + tmpPath + path.sep + baseFilename + ".isx'" +
...
let cmdImportBG = envCtx.istool + " glossary import" +
    " -af " + envCtx.authFile +
    " -f '" + tmpPath + path.sep + baseFilename + ".xml'" +
    " -format XML -mergemethod mergeoverwrite";
if (argv.mapping) {
...
if (bCreated) {
  ...
  shell.rm(tmpPath + path.sep + baseFilename + ".xml");
}
